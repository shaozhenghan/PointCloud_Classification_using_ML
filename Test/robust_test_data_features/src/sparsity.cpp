...
int main (void)
{
    // percentage of points after downsampling
    const std::vector<float> spa_percen_vec {10.0, 20.0, 30.0, 40.0, 50.0, 60.0, 70.0, 80.0, 90.0}; // %
    // path
    std::string read_file_base_path = "/media/shao/TOSHIBA EXT/data_object_velodyne/Daten/test/data_augmented";

    for (auto & sp : spa_percen_vec)
    {
        // file path to write
        std::string write_file_base_path = 
            "/media/shao/TOSHIBA EXT/data_object_velodyne/Daten/test/robust_test/sparsity";
        std::stringstream spaper;
        spaper << sp;
        ...
        for (int i = 0; i != 5; ++i)
        {
            ...
            for (auto & f : file_name_vec)
            {
                ...
                getSparseCloud(cloud_original, sp, cloud);
